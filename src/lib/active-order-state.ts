...
export function buildActiveOrderStateBlock(input: ActiveOrderStateInput): string {
  ...
  if (orderNumber) {
    ...
    lines.push(
      paid
        ? "حالة الدفع: تم تأكيد الدفع بالفعل — لا تطلب الدفع أو إثبات التحويل مرة أخرى ولا تقل إن الأوردر بانتظار الدفع."
        : "حالة الدفع: لم يتم تأكيد الدفع بعد.",
    );
    lines.push(
      "الطلب أُنشئ بنجاح بهذه البيانات بعد التحقق منها، ومرحلة جمع البيانات انتهت. لا تعيد جمع أي بيان، ولا تعيد فتح أي خطوة سابقة، ولا تقل إطلاقاً إن المنتج أو اللون أو المقاس أو الكمية أو منطقة الشحن غير متاحة لهذا الطلب. تعامل فقط مع حالته الحالية.",
    );
  } else {
